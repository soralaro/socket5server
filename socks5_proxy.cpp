...
// 辅助函数：将socket fd转换为字符串用于日志
std::string socket_to_string(int s) {
    std::stringstream ss;
    ss << s;
    return ss.str();
}
            ...
            workers.emplace_back([this, i] {
                ...
                while (true) {
                    std::function<void()> task;
                    {
                        ...
                        this->tasks.pop();
                        std::cout << "Worker thread picked up a task, remaining tasks: " 
                                  << this->tasks.size() << std::endl;
                    }
                    task();
                }
            });
...
void handle_client(int client_socket) {
    ...
    if (bytes_received < 0) {
        std::cerr << "[" << client_id << "] Failed to receive authentication data. Error: " 
    ...
    if (bytes_received == 0) {
        std::cerr << "[" << client_id << "] Client closed connection during authentication phase" 
                  << std::endl;
        close(client_socket);
        return;
    }
    
    std::cout << "[" << client_id << "] Received " << bytes_received 
              << " bytes for authentication" << std::endl;
    ...
    std::cout << "[" << client_id << "] SOCKS5 version confirmed. Supported auth methods count: " 
    ...
    if (send_result < 0) {
        std::cerr << "[" << client_id << "] Failed to send auth response. Error: " 
                  << strerror(errno) << std::endl;
        close(client_socket);
        return;
    }
    
    std::cout << "[" << client_id << "] Sent authentication response: no authentication required" 
              << std::endl;
    ...
    if (bytes_received < 0) {
        std::cerr << "[" << client_id << "] Failed to receive connection request. Error: " 
                  << strerror(errno) << std::endl;
        close(client_socket);
        return;
    }
    ...
    std::cout << "[" << client_id << "] Received " << bytes_received 
              << " bytes for connection request" << std::endl;
    ...
    if (buffer[3] == SOCKS5_ATYP_IPV4) {
        ...
        target_address = inet_ntoa(addr);
        target_port = (static_cast<unsigned char>(buffer[8]) << 8) | 
        ...
        if (bytes_received < 5 + domain_length + 2) {
    ...
    if (target_socket == -1) {
        std::cerr << "[" << client_id << "] Failed to create target socket for " 
    ...
    std::cout << "[" << client_id << "] Target socket created successfully (socket id: " 
              << target_socket << ")" << std::endl;
    ...
    if (inet_pton(AF_INET, target_address.c_str(), &target_addr.sin_addr) != 1) {
        std::cout << "[" << client_id << "] Target is not an IPv4 address, attempting DNS resolution..." 
                  << std::endl;
        ...
        if (getaddrinfo_result != 0) {
            std::cerr << "[" << client_id << "] Failed to resolve domain " << target_address 
                      << ". Error: " << gai_strerror(getaddrinfo_result) << std::endl;
            close(target_socket);
            close(client_socket);
            return;
        }
        ...
        std::cout << "[" << client_id << "] Domain " << target_address << " resolved to " 
                  << ip_str << std::endl;
    ...
    std::cout << "[" << client_id << "] Attempting to connect to target " 
              << target_address << ":" << target_port << std::endl;
    
    if (connect(target_socket, (sockaddr*)&target_addr, sizeof(target_addr)) == -1) {
        std::cerr << "[" << client_id << "] Failed to connect to target server " 
                  << target_address << ":" << target_port << ". Error: " 
                  << strerror(errno) << std::endl;
        close(client_socket);
        close(target_socket);
    ...
    // 4. 发送连接成功响应
    char connect_response[] = { SOCKS5_VERSION, 0x00, 0x00, SOCKS5_ATYP_IPV4, 
                                0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
    send_result = send(client_socket, connect_response, sizeof(connect_response), 0);
    
    if (send_result < 0) {
        std::cerr << "[" << client_id << "] Failed to send connection response. Error: " 
                  << strerror(errno) << std::endl;
        close(client_socket);
        close(target_socket);
        return;
    ...
    while (true) {
        int ret = poll(fds, 2, 300000); // 5分钟超时 (300000毫秒)
        
        if (ret < 0) {
            std::cerr << "[" << client_id << "] Poll error. Error: " << strerror(errno) << std::endl;
            break;
        }
        
        if (ret == 0) {
            std::cout << "[" << client_id << "] Connection timed out after 5 minutes of inactivity" 
                      << std::endl;
            break;
        }
        ...
        if (fds[0].revents & POLLIN) {
            ...
            if (bytes_received <= 0) {
                if (bytes_received < 0) {
                    std::cerr << "[" << client_id << "] Error receiving from client. Error: " 
                              << strerror(errno) << std::endl;
                } else {
            ...
            std::cout << "[" << client_id << "] Received " << bytes_received 
                      << " bytes from client, forwarding to target" << std::endl;
            
            int send_bytes = send(target_socket, buffer, bytes_received, 0);
            if (send_bytes < 0) {
                std::cerr << "[" << client_id << "] Failed to send to target. Error: " 
                          << strerror(errno) << std::endl;
                break;
            }
            
            if (send_bytes != bytes_received) {
                std::cerr << "[" << client_id << "] Warning: Only sent " << send_bytes 
                          << " of " << bytes_received << " bytes" << std::endl;
            }
        }
        ...
        if (fds[1].revents & POLLIN) {
            ...
            if (bytes_received <= 0) {
                if (bytes_received < 0) {
                    std::cerr << "[" << client_id << "] Error receiving from target. Error: " 
                              << strerror(errno) << std::endl;
                } else {
                    std::cout << "[" << client_id << "] Target server closed the connection" 
                              << std::endl;
                }
                break;
            }
            ...
            std::cout << "[" << client_id << "] Received " << bytes_received 
                      << " bytes from target, forwarding to client" << std::endl;
            
            int send_bytes = send(client_socket, buffer, bytes_received, 0);
            if (send_bytes < 0) {
                std::cerr << "[" << client_id << "] Failed to send to client. Error: " 
                          << strerror(errno) << std::endl;
                break;
            }
...
int main() {
    ...
    if (listen_socket == -1) {
        std::cerr << "Failed to create listen socket. Error: " << strerror(errno) << std::endl;
        return 1;
    }
    ...
    if (setsockopt(listen_socket, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) < 0) {
        std::cerr << "Failed to set SO_REUSEADDR. Error: " << strerror(errno) << std::endl;
    }
    
    sockaddr_in server_addr;
    server_addr.sin_family = AF_INET;
    server_addr.sin_addr.s_addr = INADDR_ANY;
    server_addr.sin_port = htons(1082);
    ...
    std::cout << "Socket bound to port 1082 successfully" << std::endl;
    ...
    std::cout << "SOCKS5 proxy server is running on port 1082, waiting for connections..." << std::endl;
